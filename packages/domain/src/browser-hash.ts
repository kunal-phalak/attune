...
function sha256(input: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h + sum1 + choose + ROUND_CONSTANTS[index] + words[index]) >>> 0;
...
/** Synchronous SHA-256 for browser-safe deterministic topology identifiers. */
export function browserHashCanonical(value: unknown): string {
  const serialized = JSON.stringify(canonicalize(value));
  if (serialized === undefined) throw new TypeError('Cannot hash an undefined canonical value.');
